...
var (
	...
	// ErrEmailNotVerified is returned when a token's email claim is not verified
	// and the configuration does not allow unverified emails.
	ErrEmailNotVerified = errors.New("OAuth email is not verified")
	// ErrUnauthorizedDomain is returned when a token's principal domain is not
	// in the configured allowed-email-domain / allowed-hosted-domain list.
