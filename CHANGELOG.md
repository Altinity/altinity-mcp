...
DEPENDENCY UPDATES
- bump `github.com/modelcontextprotocol/go-sdk` to 1.7.0 (stateless MCP 2026-07-28 support), `github.com/ClickHouse/clickhouse-go/v2` to 2.47.0, `github.com/urfave/cli/v3` to 3.10.1, `github.com/franchb/embedded-clickhouse` to 0.4.2, `github.com/AfterShip/clickhouse-sql-parser` to 0.5.3, `golang.org/x/crypto` to 0.54.0, `golang.org/x/net` to 0.57.0, `golang.org/x/sync` to 0.22.0
- bump `github.com/altinity/go-mcp-oauth-sdk` (adds the `UsernameClaim` / `UsernameFromExtra` API) ([PR #153](https://github.com/Altinity/altinity-mcp/pull/153))
